...
This Quarto template uses a 'uhh-template.docx' Word file, which is based on the [standard template file](https://www.kus.uni-hamburg.de/themen/oeffentlichkeitsarbeit/corporate-design/vorlagen.html) of the University of Hamburg (UHH) except for the font type, which is by default 'Lato'.  Alternatively, you can use the font style 'Georgia'. You can choose between both fonts when running the function, e.g., `ipedfmodelos::create_quarto_doc(dirname = "your_word_doc", template = "word", font = "Georgia")`.
