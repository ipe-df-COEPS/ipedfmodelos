...
date: \today
fontsize: 11pt
portuguese: true # default is Portuguese
...
* `english`: If option is set to `true`, the table and figure caption, the page information, as well as the ToC and reference header will be in English; default is `false` (i.e., Portuguese).
...
* `output`: The nested fields for the output field are based on the arguments of the output function. Since `cgmindmodelos::pdf_report` is based on `rmarkdown::pdf_document`, see its help page for more options. Current default settings are
    * `toc = TRUE`
    * `toc_depth = 5`
...
If you are associated with the UHH you can also use font "Georgia". In that case replace `font = "Lato"` with `font = "Georgia"`. To use another font, simply use the setting "other" and replace the 'font_XXX.ttf' files in the working directory with your own files. Please note, that you have to name these files exactly as the template font files.
