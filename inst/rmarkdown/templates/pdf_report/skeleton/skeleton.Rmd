...
* `output`: Os campos aninhados para o campo de saída são baseados nos argumentos da função de saída. Já que `ipedfmodelos::pdf_periodic` é baseado em `rmarkdown::pdf_document`, veja sua página de ajuda para mais opções. As configurações padrão atuais são
